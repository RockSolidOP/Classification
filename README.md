...
**Outputs**
- `page_label_scores.csv` — Per‑page per‑label scores and the baseline best label.
- `seq_smoothing_summary.csv` — Accuracy and number of switches for each tested `switch_penalty`.
- `eval_seq_smoothing_best.csv` — Final per‑page predictions using the best penalty, along with ground truth and match flag.
