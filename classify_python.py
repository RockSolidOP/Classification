...
def apply_other_rule(page_idx: int, pred_label: str) -> str:
    """Map low-confidence predictions to Other, with an optional
    fallback to generic STATEMENT if available and confident."""
...
# Minimal adjacency tie-break: if page i-1 is X and X_STMT exists,
# and score(page i, X_STMT) is within a small epsilon of the smoothed
# label's score (or higher), flip page i to X_STMT.
TIEBREAK_EPS = 0.01
best_seq_raw_tb = list(best_seq_raw)
for i in range(1, P):
    prev = best_seq_raw_tb[i-1]
    cand = f"{prev}_STMT"
    if cand in labels:
        pred = best_seq_raw_tb[i]
        c_idx = labels.index(cand)
        p_idx = labels.index(pred)
        c_score = float(score_mat[i, c_idx])
        p_score = float(score_mat[i, p_idx])
        if c_score + TIEBREAK_EPS >= p_score:
            best_seq_raw_tb[i] = cand

best_seq = [apply_other_rule(i, best_seq_raw_tb[i]) for i in range(P)]
best_acc = accuracy(best_seq)

# (no post-write modifications to page_label_scores.csv)
